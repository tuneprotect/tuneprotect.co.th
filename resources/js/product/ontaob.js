                    ...
                    case 3:
                        ...
                        <div class="two-col">
                            ...
                            ${$('#controller').value === 'product' && promotionCodeStatus
                                ? `<div class="controls-wrapper full no-lable"><span>${$('#lblfdPromotionCode').innerText} : </span><strong>${$('#fdPromotionCode').value} ${ promotion_data.result.status ? `<span id="promotion_code_alert" style="color: #008b06;">${locale === 'th' ? '(ส่วนลดสตาร์บัค 100 บาท)'`
                                ${ promotion_data.result.codeAvailable < i+1 
                                    ...
                                    : '(Starbucks card 100 bath)'}</span>` 
                                    : `<span id="promotion_code_alert" style="color: #e71618;">${locale === 'th' ? '(* โค้ดของคุณไม่เข้าเงื่อนไขโปรโมชั้นตามที่กำหนด)' : '(* The code does not qualify for the promotion as specified)'}</span>` } </strong>
                                </div>`
                                : ''
                            }
