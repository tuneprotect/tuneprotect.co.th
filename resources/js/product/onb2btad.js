                ...
                switch (parseInt(step)) {
                    ...
                    case 3:
                        ...
                        let sb = `<h3 class="text-primary">${$summary_section.getAttribute('data-insurance_data')}</h3><br/>
                            ...
                            ${$('#ctrl_travel_type').value === 'annual' ? `<div><span>YYY : </span><strong>XXX</strong></div>` : `<div><span>${$('label[for=fdDestTo]').innerText} : </span><strong>${$destTo.options[$destTo.selectedIndex].text}</strong></div>`}
