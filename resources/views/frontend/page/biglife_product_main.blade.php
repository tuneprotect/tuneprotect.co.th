...
@section('page')
    <main>
{{--        @include('frontend.component.biglife_product_list',[ 'product' => $current_product,'locale' => $locale ])--}}
        <section class="product-list wrapper reveal">
            <div class="product-list-outer-wrapper rotate-bg no-rotate">
                <div class="inner-wrapper">
                    <h1>เลือกผลิตภัณฑ์</h1>                    {{--            <h1>{{$product->locales[$locale]->title}}</h1>--}}
                    <div class="item-wrapper">
                        @foreach ($current_product->productPackage as $v)
                            <div class="item">
                                ...
                                <div class="description ">
                                    @if(strpos($v->locales[$locale]->content, 'data-gtm') === false)
                                        ...
                                        <a href="{{route('current',['locale' => $locale,'controller' => 'product','func' => $current_product->friendly_url,'params' => $v->code ])}}"
                                           data-gtm="all-product-{{strtolower($current_product->friendly_url)}}-{{$v->code}}"
