...
import WebContentDetailForm from "./WebContentDetailForm";
...
export default function WebContentMainForm({isWebContent, type_id, config, page, id, setId, parent_id = 0, setShowSection}) {
            ...
            arr = [
                {
                    ...
                    onClick: () => {
                        ...
                        if (setShowSection) {
                            setShowSection('table');
                        } else {
                            history.push(APP_ROUTE[type_id].url)
                        }
                    },
                    isNew: true
                },
    ...
    const handleSubmit = async (values) => {
        ...
        const response = await connectAPI(API_URL_LIST.webContentSave,
            {
                data: {...values, parent_id},
            });
        if (response.status === API_STATUS_RESULT.SUCCESS) {
            setFromDB(response.result);
            if (isWebContent) {
                if (APP_ROUTE?.[type_id + ".detail"]?.url) {
    ...
    const handlePublish = async () => {
        ...
        const response = await connectAPI(API_URL_LIST.webContentPublish, {
            data: {
                selected_id: id,
            }
        });
    ...
    const handleEnable = async () => {
        const response = await connectAPI(API_URL_LIST.webContentSetEnable, {
            data: {
                selected_id: id,
            }
        });
    ...
    const handleDelete = async () => {
        const response = await connectAPI(API_URL_LIST.webContentSetDelete, {
            data: {
                selected_id: id
            },
        });
    ...
    useEffect(() => {
        (async () => {
            ...
            if (isWebContent && id) {
                where.id = id;
            } else if (['static.page', 'static.meta', 'static.email', 'static.home'].some(v => type_id.startsWith(v))) {
                where.type_id = type_id;
            }

            if (Object.keys(where).length > 0) {
                const response = await connectAPI(API_URL_LIST.webContentGet, {
                    data: where,
                    showStatus: false
                });
    ...
    return <>
        {{
            'detail': <WebContentDetailForm {...childProp} />,
            'seo': <SEOPage {...childProp} />,
