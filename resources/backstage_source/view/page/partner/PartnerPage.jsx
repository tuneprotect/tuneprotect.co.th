...
export default function PartnerPage({type_id, cat_id, config}) {
    ...
    const handleEnable = (id, status) => {
        handleAction(API_URL_LIST.partnerSetEnable, 'enable', id, status)
    };
    const handleDelete = async (id, status) => {
        handleAction(API_URL_LIST.partnerSetDelete, 'mark_delete', id, status)
    };

    const handlePublish = async (id, status) => {
        handleAction(API_URL_LIST.partnerPublish, 'publish', id, status)
    };

    const handleReload = async () => {

        const response = await connectAPI(API_URL_LIST.partnerGetAll, {
            data: {type_id},
            showStatus: false
        });
        if (response.status === API_STATUS_RESULT.SUCCESS) {
    ...
    const handleSaveReorder = async (data) => {
        ...
        const response = await connectAPI(API_URL_LIST.partnerSaveReorder, {
